...
def test(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        #print(f'test.py 118 : shape {img.shape}')
    ...
    if subm == True:
        ...
        def box_list_(dict_data):
            '''
            이 함수는 위에서 나온 결과 가지고 submission.csv -> PredictionString에
            들어가는 부분을 만들기 위해 사용합니다.
            d[0] = {'image_id': 0, 'category_id': 8, 'bbox': [601.0, 519.0, 355.0, 505.0], 'score': 0.979}
            ...
            for i in range(len(jdict)):
                ...
                while True:
                    ...
                    if len(d) == 1:
                        ...
                        box_list.append(d[0]['score'])
                        box_list.append(str((d[0]['bbox'][0])))
        ...
        def csv_(box_list_):
            '''
            위 box_list만들어주는 함수에서 box_list를 받아 submission에 들어갈 한 행씩 만들어줍니다.
            '''
            print('In --- csv_ ---')
            res = []
            for i in range(len(box_list_)):
                res.append({'index':i, 'PredictionString':box_list_[i], 'image_id':'test/' + str(i).zfill(4)})
            return res


        def sbm_csv(dict_data):
            '''
            DataFrame을 만들고, 위에서 만든 딕셔너리 리스트를 받아서 
            각 열에 넣어줍니다. 
            '''
            print('In --- sbm_csv ---')
            sbm = pd.DataFrame(columns=['index','PredictionString','image_id'])
            box_temp = []
            index_temp = []
            id_temp = []
            for i in range(len(dict_data)):
                e = ''
                for j in range(len(dict_data[i]['PredictionString'])):
                    e += str(dict_data[i]['PredictionString'][j]) + ' '
                box_temp.append(e[:-1])
                index_temp.append(i)
                id_temp.append(dict_data[i]['image_id'] + '.jpg')
            sbm['PredictionString'] = box_temp
            sbm['index'] = index_temp
            sbm['image_id'] = id_temp
            return sbm
        ...
        sbm_test.to_csv('./fin_iou_0_5_yolor_submission_mo9_stratified_SGD.csv', index=None)
...
if __name__ == '__main__':
    ...
    # 추가
    parser.add_argument('--subm', type=str, default='no', help='yes is get submission.csv')
