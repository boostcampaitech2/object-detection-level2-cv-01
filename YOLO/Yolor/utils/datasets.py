...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # print(f'self.image_weights : {self.image_weights}')
        # print(f'self.rect : {self.rect}')
        # print(f'self.mosaic : {self.mosaic}')
        # print(f'self.mosaic_border : {self.mosaic_border}')
        # print(f'self.stride : {self.stride}')
        # print('-_'*30)
        # print(os.sep)
        # print('-_'*30)
        def img2label_paths(img_paths):
            #print('In Img2label_paths')
        ...
        try:
            ...
            for p in path if isinstance(path, list) else [path]:
                #print(f'pppppppppppppppppppppp: {p}')
                # p -> /opt/ml/detection/dataset/train_kfold1.txt
                p = Path(p)  # os-agnostic , Path(p) -> WindowsPath('/opt/ml/detection/dataset/train_kfold1.txt')
                #print(p)
                #pdb.set_trace()
                if p.is_dir():  # dir
...
def load_image(self, index):
    ...
    if img is None:  # not cached
        ...
        if r != 1:  # always resize down, only resize up if training with augmentation
            ...
            img = cv2.resize(img, (int(w0 * r), int(h0 * r)), interpolation=interp) # 사이즈 1280으로 맞춰줌?
